...
def draw_statevector_grid(screen):
    ...
    for i in range(len(basis_states)):
        text = font.vector_font.render(basis_states[i], 1, globals.LIGHT_BLUE)
...
def draw_score(screen, classical_score, quantum_score):
    ...
    text = font.player_font.render("Classical Computer", 1, globals.DUSTY_RED)
    ...
    text = font.player_font.render("Quantum Computer", 1, globals.DUSTY_BLUE)
    text_pos = text.get_rect(center=(globals.WINDOW_WIDTH*0.7, globals.WIDTH_UNIT*2))
    screen.blit(text, text_pos)

    text = font.score_font.render(str(quantum_score), 1, globals.BLUE)
